use kallsyms_tools::{ldscript::ldscript, symbol::symbols_from_file};
...
fn main() {
    ...
    for i in 0..4 {
        ...
        if i > 0 {
            ...
            file.sync_all().expect("Failed to write linker script file");

            let tmp_path_ld = tmp_path_ld
                .to_str()
        ...
        let status = linker.status().expect("Failed to execute linker process");
        ...
        if i > 0 {
            ...
            if curr_syms == prev_syms {
                success = true;
                fs::rename(tmp_path_bin, out_path_original).expect("Failed to rename output file");
                break;
            }
        }
