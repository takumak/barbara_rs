...
use raw::{header::ElfHeader, section_header::ElfSectionHeader};
...
impl<'a> ElfParser<'a> {
    pub fn from_bytes(data: &'a [u8]) -> Result<Self, ElfParserError> {
        use raw::{
            ...
            ident::parse_ident,
            section_header::{Elf32SectionHeader, Elf64SectionHeader},
        };
    ...
    {
        use raw::{section_parser::SectionParser, strtab};
        ...
        for idx in 0..parser.header.get_shnum() {
            ...
            let name = strtab::read_at(strtab_data, sh.get_name() as usize);
            let sec = ElfSection {
                name,
                typ: sh.get_type(),
                flags: sh.get_flags(),
    ...
    pub fn iter_symbols(&'a self) -> ElfSymtabIterator<'a> {
        ElfSymtabIterator::new(self.class, self.endian, &self.sections)
    }
}

#[cfg(test)]
mod tests {
    use crate::{ElfClass, ElfEndian, ElfParser, ElfSection, ElfSectionHeaderType, ElfSymbol};
    ...
    fn elf32be() {
        let data: &[u8] = &[
            ...
            0, b'.', b's', b'h', b's', b't', b'r', b't', b'a', b'b', 0,
        ];
        ...
        assert_eq!(
            ...
            vec![ElfSection {
                ...
                addralign: 1,
                entsize: 0,
                content: &[0, b'.', b's', b'h', b's', b't', b'r', b't', b'a', b'b', 0,],
            }]
        );
    }
    ...
    fn elf32be_section_index_out_of_range() {
        ...
        ElfParser::from_bytes(&data).expect_err("ElfParser::from_bytes unexpectedly succeed");
    }
    ...
    fn elf64le_iter_symbols() {
        let data: &[u8] = &[
            ...
            0, b'.', b's', b'h', b's', b't', b'r', b't', b'a', b'b', 0, b'.', b's', b't', b'r',
            b't', b'a', b'b', 0, b'.', b's', b'y', b'm', b't', b'a', b'b', 0, 0, 0, 0, 0, 0,
            // +0120 .strtab section content
            0, b't', b'e', b's', b't', b'1', 0, b't', b'e', b's', b't', b'2', 0, 0, 0, 0,
        ...
        let p = ElfParser::from_bytes(&data).expect("ElfParser::from_bytes failed");
        ...
        assert_eq!(
            ...
            ElfSection {
                ...
                entsize: 0,
                content: &[
                    0, b'.', b's', b'h', b's', b't', b'r', b't', b'a', b'b', 0, b'.', b's', b't',
                    b'r', b't', b'a', b'b', 0, b'.', b's', b'y', b'm', b't', b'a', b'b', 0,
                ]
            }
        );
        ...
        assert_eq!(
            ...
            ElfSection {
                ...
                addralign: 1,
                entsize: 0,
                content: &[0, b't', b'e', b's', b't', b'1', 0, b't', b'e', b's', b't', b'2', 0,]
            }
        );
        ...
        let syms: Vec<ElfSymbol> = p.iter_symbols().map(|r| r.unwrap()).collect();
    ...
    fn elf32be_nobits() {
        let data: &[u8] = &[
            ...
            0, 0, 0, 0, // entsize
            // .shstrtab section content
            0, b'.', b's', b'h', b's', b't', b'r', b't', b'a', b'b', 0, b'.', b'b', b's', b's', 0,
        ];
        ...
        assert_eq!(
            ...
            vec![
                ElfSection {
                    ...
                    entsize: 0,
                    content: &[
                        0, b'.', b's', b'h', b's', b't', b'r', b't', b'a', b'b', 0, b'.', b'b',
                        b's', b's', 0
                    ],
                },
                ElfSection {
