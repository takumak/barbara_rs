...
pub(crate) fn parse_ident(data: &[u8]) -> Result<(ElfClass, ElfEndian), ElfParserError> {
    if data.len() < ELF_IDENT_SIZE {
        return Err(ElfParserError::new(
            Errno::EINVAL,
            String::from("File size too small"),
        ));
    }

    let ident = <[u8; ELF_IDENT_SIZE]>::try_from(&data[..ELF_IDENT_SIZE]).unwrap();

    if ident[..4] != MAGIC {
        return Err(ElfParserError::new(
            Errno::EINVAL,
            String::from("Magic number mismatch"),
        ));
    }
    ...
    let endian = match ident[OFF_DATA] {
        ...
        d => {
            return Err(ElfParserError::new(
                Errno::EINVAL,
                format!("Invalid elf data format: {}", d),
            ))
        }
    };
    ...
    if ident[OFF_VERSION] != EV_CURRENT {
        return Err(ElfParserError::new(
            Errno::EINVAL,
            format!("Unknown elf version: {}", ident[OFF_VERSION]),
        ));
    }
...
mod tests {
    ...
    fn invalid_data() {
        parse_ident(&[
            ...
            1u8,  // 1: 32bit, 2: 64bit, others: error
            0u8,  /* error */
            // 1: Little endian, 2: Big endian, others: error
            1u8, // elf version; should be 1
            3u8, // OS ABI
            0u8, // ABI version
            0u8, 0u8, 0u8, // padding
