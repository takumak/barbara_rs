...
use crate::{ElfEndian, ElfParserError, ElfSectionHeaderType};

use crate::raw::{header::ElfHeader, ident::ELF_IDENT_SIZE, section_header::ElfSectionHeader};
...
where
    ...
    SH: Stpack + ElfSectionHeader,
{
    pub fn new(data: &[u8], endian: ElfEndian) -> Result<Self, ElfParserError> {
    ...
    pub fn nth<'a>(&self, data: &'a [u8], idx: usize) -> Result<(SH, &'a [u8]), ElfParserError> {
        ...
        if data.len() < off + size {
            return Err(ElfParserError::new(
                Errno::EINVAL,
                format!(
                    "Elf section header out of range: \
        ...
        let sh = SH::unpack(&data[off..(off + size)], self.le).or(Err(ElfParserError::new(
            Errno::EINVAL,
            format!("Failed to parse elf section header: {}", idx),
        )))?;

        let content = if sh.get_type() != ElfSectionHeaderType::Nobits {
            ...
            if data.len() < (off + size) {
                return Err(ElfParserError::new(
                    Errno::EINVAL,
                    format!(
                        "Elf section content out of range: \
                                 section={:#x}--{:#x}, filesize={:#x}",
                        off,
                        off + size,
                        data.len()
...
mod tests {
    ...
    fn elf32_incomplete_header() {
        ...
        let parser = Parser::new(&[0; 17], ElfEndian::ElfBE);
